...
app.post("/register", async (req, res) => {
  ...
  //generate a token
  const verficationtoken = generateJWTToken(email);
...
app.get("/verify/:id", (req, res) => {
  ...
  jwt.verify(token, "this is jan nodejs class", async (err, verifiedJwt) => {
    if (err) throw err;
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (userDb && !userDb.emailAuthenticated) {
      return res.send({
        status: 402,
        message: "Email is not authenticated, Please verify before login",
      });
    }
